...
Publishing (OIDC)**: no npm tokens, automatic provenance attestations
(`.github/workflows/release.yml`). Also carries the post-merge CI fixes
(prettier gate now enforced locally, case-sensitive migrator known-files,
retired terse sync workflow removed).
...
- New `docs/usage-examples.md` with end-to-end examples for every workflow
  ...
  `rulebook claude` command and the spec → driver flow; the Documentation
  section links the new guide.
...
- **Lean AGENTS.md is now unconditional.** `generateFullAgents` and `mergeFullAgents` always emit the condensed ~211-line template regardless of `agentsMode` or `modular` config values. Users with `agentsMode: 'full'` from older versions will get the lean output on next `rulebook update`. Dead legacy codepath (procedural 6k-line generator) removed.
...
- **Auto-migrate legacy archives on every task operation**: `TaskManager.initialize()` now calls `migrateArchive()` automatically, moving any remaining archives from `.rulebook/tasks/archive/` to `.rulebook/archive/`. Previously migration only ran during `rulebook update`, leaving stale archives in the legacy location.
- **`.gitignore` updated**: Added `!/.rulebook/archive/` to track archived tasks in the new location.
