...
describe('ConfigManager', () => {
    ...
    describe('toggleFeature', () => {
        it('should enable feature', async () => {
            ...
            const config = await configManager.toggleFeature('health', true);

            expect(config.features.health).toBe(true);
        });

        it('should disable feature', async () => {
            await configManager.initializeConfig();

            const config = await configManager.toggleFeature('templates', false);

            expect(config.features.templates).toBe(false);
        });
    });
