...
describe('opencode-generator', () => {
  ...
  describe('normalizeOpencodeSkillName', () => {
    it('lowercases and hyphenates', () => {
      expect(normalizeOpencodeSkillName('Rulebook_Terse_Commit')).toBe('rulebook-terse-commit');
    });
  ...
  describe('generateOpencodeConfig', () => {
    ...
    it('preserves user keys when opencode.json already exists', async () => {
      await fs.writeFile(
        ...
        JSON.stringify({ model: 'anthropic/claude-opus-4-7', theme: 'dark', custom: true }, null, 2)
      );
