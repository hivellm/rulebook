...
describe('validator', () => {
    ...
    describe('validateProject', () => {
        ...
        it('should fail when /tests directory is missing', async () => {
            ...
            expect(
                result.errors.some((e) => e.message.includes('/tests directory not found'))
            ).toBe(true);
        });
        ...
        it('should warn when /docs directory is missing', async () => {
            ...
            expect(
                result.warnings.some((w) => w.message.includes('/docs directory not found'))
            ).toBe(true);
        });
        ...
        it('should warn when important docs are missing', async () => {
            ...
            expect(result.warnings.some((w) => w.message.includes('ROADMAP.md not found'))).toBe(
                true
            );
        });
        ...
        it('should handle nested test directories', async () => {
            ...
            await fs.writeFile(
                path.join(testDir, 'tests', 'integration', 'test.test.js'),
                '// test'
            );
    ...
    describe('formatValidationReport', () => {
        ...
        it('should generate clean report when all checks pass', () => {
            ...
            expect(report).toContain(
                '✅ All checks passed! Your project follows rulebook standards.'
            );
        });
    ...
    describe('validateRulebookDirectory edge cases', () => {
        it('should warn when rulebook directory exists but is empty', async () => {
            ...
            expect(
                result.warnings.some((w) =>
                    w.message.includes(
                        '.rulebook/specs/ directory exists but contains no .md files'
                    )
                )
            ).toBe(true);
        });
        ...
        it('should not warn when rulebook directory has .md files', async () => {
            ...
            await fs.mkdir(path.join(testDir, '.rulebook', 'specs'), { recursive: true });
            await fs.writeFile(
                path.join(testDir, '.rulebook', 'specs', 'TYPESCRIPT.md'),
                '# TypeScript'
            );
    ...
    describe('validateDocumentation edge cases', () => {
        ...
        it('should warn for each missing important doc', async () => {
            ...
            expect(
                result.warnings.some((w) => w.message.includes('ARCHITECTURE.md not found'))
            ).toBe(true);
        });
    ...
    describe('validateRulesIgnore edge cases', () => {
        ...
        it('should not warn about non-wildcard patterns', async () => {
            ...
            expect(result.warnings.some((w) => w.message.includes('ignores all rules'))).toBe(
                false
            );
        });
    });
