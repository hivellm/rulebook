...
export async function initCommand(options: {
    ...
    try {
        ...
        // Language + library selection. Interactive terminals (and not --yes) get a
        ...
        if (detection.opencode?.detected) {
            try {
                ...
                if (oc.agents.length > 0) {
                    console.log(
                        chalk.gray(
                            `  • OpenCode agents: ${oc.agents.length} written to .opencode/agents/`
                        )
                    );
                }
                if (oc.skills.length > 0) {
                    console.log(
                        chalk.gray(
                            `  • OpenCode skills: ${oc.skills.length} written to .opencode/skills/`
                        )
                    );
                }
                if (oc.preserved.length > 0) {
                    console.log(
                        chalk.gray(
                            `  · ${oc.preserved.length} user-owned OpenCode file(s) preserved`
                        )
                    );
                }
            } catch (err) {
            ...
            try {
                ...
                if (result.detected) {
                    ...
                    if (result.agentTeamsEnabled) {
                        console.log(
                            chalk.gray('  • Multi-agent teams enabled in .claude/settings.json')
                        );
                    }
                    ...
                    if (result.workflowDefinitionsInstalled.length > 0) {
                        console.log(
                            chalk.gray(
                                `  • ${result.workflowDefinitionsInstalled.length} workflows installed to .claude/workflows/`
                            )
                        );
                    }
                } else {
        ...
        try {
            ...
            if (created) {
                console.log(
                    chalk.gray('  • AGENTS.override.md created (add project-specific rules here)')
                );
            }
        } catch {
            // Non-blocking
