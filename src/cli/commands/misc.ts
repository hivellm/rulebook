...
export async function validateCommand(): Promise<void> {
    try {
        ...
        if (result.valid && result.warnings.length === 0) {
            ...
            console.log(
                chalk.yellow(
                    `✅ Project is valid but has ${result.warnings.length} warnings to address.\n`
                )
            );
        } else {
            console.log(
                chalk.red(`❌ Project has ${result.errors.length} errors that must be fixed.\n`)
            );
...
export async function workflowsCommand(): Promise<void> {
    try {
        ...
        if (detection.languages.length === 0) {
            ...
            console.log(
                chalk.gray('Ensure you have project files (Cargo.toml, package.json, etc.)\n')
            );
            return;
        }
...
export async function checkDepsCommand(): Promise<void> {
    try {
        ...
        if (result.vulnerable.length > 0) {
            ...
            console.log('');
            console.log(
                chalk.red.bold('⚠️  SECURITY: Update vulnerable dependencies immediately!')
            );
        }
...
export async function checkCoverageCommand(options: { threshold?: number }): Promise<void> {
    try {
        ...
        if (result.meetsThreshold) {
            ...
            console.log(
                chalk.red(
                    `❌ Coverage ${result.percentage.toFixed(2)}% is below threshold ${threshold}%\n`
                )
            );
...
export async function versionCommand(options: {
    ...
    try {
        ...
        console.log(
            chalk.gray(
                `  4. Tag: git tag -a v${result.newVersion} -m "Release v${result.newVersion}"`
            )
        );
    } catch (error) {
...
export async function configCommand(options: {
    ...
    try {
        ...
        if (options.show) {
            ...
            console.log(
                chalk.white(`CLI Tools: ${summary.cliTools.join(', ') || 'None detected'}`)
            );
            ...
            console.log(
                chalk.green(
                    `✅ Feature '${options.feature}' ${options.enable ? 'enabled' : 'disabled'}`
                )
            );
        } else if (options.set) {
