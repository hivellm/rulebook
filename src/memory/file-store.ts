...
function metaToMemory(meta: Record<string, unknown>, body: string): Memory | null {
    ...
    const type =
        typeof meta.type === 'string' && MEMORY_TYPES.has(meta.type)
...
export class FileStore {
    ...
    async getStats(): Promise<{
        ...
        for (const m of memories) {
            if (oldestMemory === undefined || m.createdAt < oldestMemory)
                oldestMemory = m.createdAt;
            if (newestMemory === undefined || m.createdAt > newestMemory)
                newestMemory = m.createdAt;
        }
        return {
