...
export class CodeParser {
  ...
  private parseTypeScript(
    ...
    if (currentChunk.trim().length > 0) {
      nodes.push(
        this.createNode(filePath, 'chunk', `chunk-end`, currentChunk, startLine, lines.length, now)
      );
    }
  ...
  private parseMarkdown(
    ...
    if (currentChunk.trim().length > 0) {
      nodes.push(
        this.createNode(filePath, 'unknown', `md-end`, currentChunk, startLine, lines.length, now)
      );
    }
