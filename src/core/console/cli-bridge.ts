        ...
        try {
            if (toolName === 'cursor-agent') {
                ...
                if (this.onLog) {
                    ...
                    this.onLog(
                        'info',
                        '   (This may take 30-60 seconds to connect to remote server)'
                    );
                } else {
                ...
                const progressInterval = setInterval(() => {
                    ...
                    if (!this.onLog) {
                        process.stdout.write(
                            '\r⏳ Waiting' + '.'.repeat(dots) + ' '.repeat(3 - dots)
                        );
                    }
                }, 500);
                ...
                const result = await new Promise<{
                    ...
                    completionPromise.then(async () => {
                        ...
                        if (!proc.killed && proc.exitCode === null) {
                            await this.debugLog(
                                `Forcing process kill immediately after completion`
                            );
                            try {
                                proc.kill('SIGTERM');
                        ...
                        setTimeout(async () => {
                            ...
                            if (!proc.killed) {
                                await this.debugLog(
                                    `Process STILL alive after 500ms, forcing SIGKILL`
                                );
                                try {
                                    proc.kill('SIGKILL');
                                } catch (error) {
